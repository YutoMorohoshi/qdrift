OPENQASM 3.0;
include "stdgates.inc";
gate rxx(_gate_p_0) _gate_q_0, _gate_q_1 {
  h _gate_q_0;
  h _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  rz(0.177) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  h _gate_q_1;
  h _gate_q_0;
}
gate rzz(_gate_p_0) _gate_q_0, _gate_q_1 {
  cx _gate_q_0, _gate_q_1;
  rz(0.177) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
}
gate unitary _gate_q_0, _gate_q_1 {
  U(pi/2, -3*pi/4, -pi) _gate_q_0;
  U(1.1848501784590992, -0.7024688891647095, -0.41842732511528613) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi, -0.46111590409721526, 1.1096804226976813) _gate_q_0;
  U(1.598743312216013, 1.7564777444264452, -1.8687473474700944) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, -pi, -pi/4) _gate_q_0;
  U(2.2125215592342102, 2.2977102204433244, -2.6523928776916197) _gate_q_1;
}
gate unitary_0 _gate_q_0, _gate_q_1 {
  U(pi/2, -pi, 0) _gate_q_0;
  U(pi/2, 0, 2.5802058126763807) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(2.9645926535897935, 0, pi/2) _gate_q_0;
  U(1.6328906019078662, 1.8442992687112794, -2.0147362454746425) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, 0, -pi/2) _gate_q_0;
  U(pi/2, -1.0094094858814837, -pi) _gate_q_1;
}
gate unitary_1 _gate_q_0, _gate_q_1 {
  U(pi/2, -3*pi/4, -pi) _gate_q_0;
  U(1.1848501784590992, -0.7024688891647095, -0.41842732511528613) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi, -0.46111590409721526, 1.1096804226976813) _gate_q_0;
  U(1.6328906019078662, 1.8442992687112794, -2.0147362454746425) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, -pi, -pi/4) _gate_q_0;
  U(2.2125215592342102, 2.2977102204433235, -2.6523928776916197) _gate_q_1;
}
gate unitary_2 _gate_q_0, _gate_q_1 {
  U(pi, 0, -pi) _gate_q_0;
  U(0.5613868409134123, pi/2, pi/2) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(2.4335926535897934, 0, pi/2) _gate_q_0;
  U(1.6793172645405703, 1.9265011683568485, -2.1576709505769402) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(0, 2.302150075581663, 2.410238904803027) _gate_q_0;
  U(1.0094094858814837, -pi/2, -pi/2) _gate_q_1;
}
gate unitary_3 _gate_q_0, _gate_q_1 {
  U(pi/2, -pi, -pi) _gate_q_0;
  U(pi/2, 0, -0.5613868409134124) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(2.7875926535897935, 0, pi/2) _gate_q_0;
  U(1.7368170247384793, 2.0017592205250017, -2.2973080546749856) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, -pi, -pi/2) _gate_q_0;
  U(pi/2, 2.13218316770831, -pi) _gate_q_1;
}
gate unitary_4 _gate_q_0, _gate_q_1 {
  U(pi/2, -3*pi/4, -pi) _gate_q_0;
  U(1.1848501784590992, -0.7024688891647095, -0.41842732511528613) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi, -0.46111590409721526, 1.1096804226976813) _gate_q_0;
  U(1.6328906019078662, 1.8442992687112794, -2.0147362454746425) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, -pi, -pi/4) _gate_q_0;
  U(2.2125215592342102, 2.2977102204433235, -2.6523928776916197) _gate_q_1;
}
gate unitary_5 _gate_q_0, _gate_q_1 {
  U(pi, 0, -pi) _gate_q_0;
  U(0.5613868409134123, pi/2, pi/2) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(2.610592653589793, 0, pi/2) _gate_q_0;
  U(1.6328906019078662, 1.8442992687112785, -2.0147362454746425) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(0, 2.302150075581663, 2.410238904803027) _gate_q_0;
  U(1.0094094858814837, -pi/2, -pi/2) _gate_q_1;
}
bit[1] c;
qubit[4] q;
rxx(0.177) q[1], q[2];
rzz(0.177) q[1], q[2];
rzz(0.177) q[1], q[2];
rxx(0.177) q[0], q[1];
rzz(0.177) q[0], q[1];
rxx(0.177) q[2], q[3];
unitary q[1], q[2];
rzz(0.177) q[0], q[1];
rzz(0.177) q[2], q[3];
rxx(0.177) q[1], q[2];
rxx(0.177) q[2], q[3];
rzz(0.177) q[1], q[2];
unitary_0 q[0], q[1];
rzz(0.177) q[2], q[3];
rzz(0.177) q[2], q[3];
rzz(0.177) q[2], q[3];
rzz(0.177) q[2], q[3];
rzz(0.177) q[2], q[3];
rzz(0.177) q[2], q[3];
rxx(0.177) q[2], q[3];
rzz(0.177) q[1], q[2];
rxx(0.177) q[1], q[2];
rxx(0.177) q[1], q[2];
rzz(0.177) q[2], q[3];
rzz(0.177) q[1], q[2];
rzz(0.177) q[1], q[2];
rxx(0.177) q[0], q[1];
rxx(0.177) q[2], q[3];
rzz(0.177) q[1], q[2];
rxx(0.177) q[0], q[1];
rzz(0.177) q[0], q[1];
rzz(0.177) q[0], q[1];
rzz(0.177) q[2], q[3];
rzz(0.177) q[2], q[3];
rzz(0.177) q[1], q[2];
rzz(0.177) q[0], q[1];
unitary_1 q[1], q[2];
rxx(0.177) q[0], q[1];
rzz(0.177) q[1], q[2];
rzz(0.177) q[1], q[2];
unitary_2 q[0], q[1];
unitary_3 q[2], q[3];
rxx(0.177) q[1], q[2];
unitary_4 q[0], q[1];
rxx(0.177) q[1], q[2];
rzz(0.177) q[1], q[2];
rxx(0.177) q[1], q[2];
rzz(0.177) q[2], q[3];
rzz(0.177) q[1], q[2];
rzz(0.177) q[0], q[1];
rxx(0.177) q[0], q[1];
rxx(0.177) q[0], q[1];
rzz(0.177) q[0], q[1];
rxx(0.177) q[2], q[3];
rzz(0.177) q[2], q[3];
rzz(0.177) q[1], q[2];
rxx(0.177) q[0], q[1];
unitary q[2], q[3];
rzz(0.177) q[1], q[2];
rxx(0.177) q[0], q[1];
unitary_5 q[2], q[3];
rxx(0.177) q[1], q[2];
rxx(0.177) q[0], q[1];
rxx(0.177) q[1], q[2];
rxx(0.177) q[0], q[1];
rxx(0.177) q[1], q[2];
rzz(0.177) q[1], q[2];
rzz(0.177) q[1], q[2];
rzz(0.177) q[0], q[1];
rzz(0.177) q[0], q[1];
rzz(0.177) q[2], q[3];
rxx(0.177) q[2], q[3];
rzz(0.177) q[2], q[3];
c[0] = measure q[0];
