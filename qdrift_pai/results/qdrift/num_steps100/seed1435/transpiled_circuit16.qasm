OPENQASM 3.0;
include "stdgates.inc";
gate rxx(_gate_p_0) _gate_q_0, _gate_q_1 {
  h _gate_q_0;
  h _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  rz(0.051000000000000004) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  h _gate_q_1;
  h _gate_q_0;
}
gate rzz(_gate_p_0) _gate_q_0, _gate_q_1 {
  cx _gate_q_0, _gate_q_1;
  rz(0.051000000000000004) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
}
gate unitary _gate_q_0, _gate_q_1 {
  U(pi/2, -3*pi/4, -pi) _gate_q_0;
  U(1.1848501784590992, -0.7024688891647095, -0.41842732511528613) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi, -0.46111590409721526, 1.1096804226976813) _gate_q_0;
  U(1.573138592541752, 1.6250297396335505, -1.657098809010133) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, -pi, -pi/4) _gate_q_0;
  U(2.2125215592342102, 2.2977102204433235, -2.6523928776916197) _gate_q_1;
}
gate unitary_0 _gate_q_0, _gate_q_1 {
  U(pi/2, 0, -pi) _gate_q_0;
  U(pi/2, -pi, 0.5613868409134124) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(3.0905926535897934, 0, pi/2) _gate_q_0;
  U(1.5760607342448538, 1.6520205088257764, -1.7001711988183779) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, -pi, pi/2) _gate_q_0;
  U(pi/2, -2.132183167708309, 0) _gate_q_1;
}
gate unitary_1 _gate_q_0, _gate_q_1 {
  U(pi/2, -3*pi/4, -pi) _gate_q_0;
  U(1.1848501784590992, -0.7024688891647095, -0.41842732511528613) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi, -0.46111590409721526, 1.1096804226976813) _gate_q_0;
  U(1.5760607342448538, 1.6520205088257764, -1.7001711988183779) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, -pi, -pi/4) _gate_q_0;
  U(2.2125215592342102, 2.2977102204433244, -2.6523928776916197) _gate_q_1;
}
gate unitary_2 _gate_q_0, _gate_q_1 {
  U(pi, 0, -pi) _gate_q_0;
  U(0.5613868409134123, pi/2, pi/2) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(2.937592653589793, 0, pi/2) _gate_q_0;
  U(1.580141169433013, 1.6788611303995573, -1.743150178589703) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(0, 3.119679514543752, 1.5927094658409375) _gate_q_0;
  U(1.0094094858814837, -pi/2, -pi/2) _gate_q_1;
}
gate unitary_3 _gate_q_0, _gate_q_1 {
  U(pi/2, -pi, 0) _gate_q_0;
  U(pi/2, 0, 2.5802058126763807) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(3.0395926535897932, 0, pi/2) _gate_q_0;
  U(1.585369466095859, 1.7055031558976417, -1.786006605392095) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, 0, -pi/2) _gate_q_0;
  U(pi/2, -1.0094094858814842, -pi) _gate_q_1;
}
gate unitary_4 _gate_q_0, _gate_q_1 {
  U(pi/2, -3*pi/4, -pi) _gate_q_0;
  U(1.1848501784590992, -0.7024688891647095, -0.41842732511528613) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi, -0.46111590409721526, 1.1096804226976813) _gate_q_0;
  U(1.5760607342448538, 1.6520205088257764, -1.7001711988183779) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, -pi, -pi/4) _gate_q_0;
  U(2.2125215592342102, 2.2977102204433244, -2.6523928776916197) _gate_q_1;
}
gate unitary_5 _gate_q_0, _gate_q_1 {
  U(pi, -0.14189705460416402, -0.14189705460416402) _gate_q_0;
  U(0.5613868409134124, pi/2, -pi/2) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(2.988592653589793, 0, pi/2) _gate_q_0;
  U(1.5760607342448538, 1.6520205088257764, -1.7001711988183779) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(0, -0.23617363547518133, 1.8069699622700774) _gate_q_0;
  U(1.0094094858814837, pi/2, -pi/2) _gate_q_1;
}
bit[1] c;
qubit[4] q;
rxx(0.051000000000000004) q[1], q[2];
rzz(0.051000000000000004) q[1], q[2];
rzz(0.051000000000000004) q[1], q[2];
rxx(0.051000000000000004) q[0], q[1];
rzz(0.051000000000000004) q[0], q[1];
rxx(0.051000000000000004) q[2], q[3];
unitary q[1], q[2];
rzz(0.051000000000000004) q[0], q[1];
rzz(0.051000000000000004) q[2], q[3];
rxx(0.051000000000000004) q[1], q[2];
rxx(0.051000000000000004) q[2], q[3];
rzz(0.051000000000000004) q[1], q[2];
unitary_0 q[0], q[1];
rzz(0.051000000000000004) q[2], q[3];
rzz(0.051000000000000004) q[2], q[3];
rzz(0.051000000000000004) q[2], q[3];
rzz(0.051000000000000004) q[2], q[3];
rzz(0.051000000000000004) q[2], q[3];
rzz(0.051000000000000004) q[2], q[3];
rxx(0.051000000000000004) q[2], q[3];
rzz(0.051000000000000004) q[1], q[2];
rxx(0.051000000000000004) q[1], q[2];
rxx(0.051000000000000004) q[1], q[2];
rzz(0.051000000000000004) q[2], q[3];
rzz(0.051000000000000004) q[1], q[2];
rzz(0.051000000000000004) q[1], q[2];
rxx(0.051000000000000004) q[0], q[1];
rxx(0.051000000000000004) q[2], q[3];
rzz(0.051000000000000004) q[1], q[2];
rxx(0.051000000000000004) q[0], q[1];
rzz(0.051000000000000004) q[0], q[1];
rzz(0.051000000000000004) q[0], q[1];
rzz(0.051000000000000004) q[2], q[3];
rzz(0.051000000000000004) q[2], q[3];
rzz(0.051000000000000004) q[1], q[2];
rzz(0.051000000000000004) q[0], q[1];
unitary_1 q[1], q[2];
rxx(0.051000000000000004) q[0], q[1];
rzz(0.051000000000000004) q[1], q[2];
rzz(0.051000000000000004) q[1], q[2];
unitary_2 q[0], q[1];
unitary_3 q[2], q[3];
rxx(0.051000000000000004) q[1], q[2];
unitary_4 q[0], q[1];
rxx(0.051000000000000004) q[1], q[2];
rzz(0.051000000000000004) q[1], q[2];
rxx(0.051000000000000004) q[1], q[2];
rzz(0.051000000000000004) q[2], q[3];
rzz(0.051000000000000004) q[1], q[2];
rzz(0.051000000000000004) q[0], q[1];
rxx(0.051000000000000004) q[0], q[1];
rxx(0.051000000000000004) q[0], q[1];
rzz(0.051000000000000004) q[0], q[1];
rxx(0.051000000000000004) q[2], q[3];
rzz(0.051000000000000004) q[2], q[3];
rzz(0.051000000000000004) q[1], q[2];
rxx(0.051000000000000004) q[0], q[1];
unitary q[2], q[3];
rzz(0.051000000000000004) q[1], q[2];
rxx(0.051000000000000004) q[0], q[1];
unitary_5 q[2], q[3];
rxx(0.051000000000000004) q[1], q[2];
rxx(0.051000000000000004) q[0], q[1];
rxx(0.051000000000000004) q[1], q[2];
rxx(0.051000000000000004) q[0], q[1];
rxx(0.051000000000000004) q[1], q[2];
rzz(0.051000000000000004) q[1], q[2];
rzz(0.051000000000000004) q[1], q[2];
rzz(0.051000000000000004) q[0], q[1];
rzz(0.051000000000000004) q[0], q[1];
rzz(0.051000000000000004) q[2], q[3];
rxx(0.051000000000000004) q[2], q[3];
rzz(0.051000000000000004) q[2], q[3];
c[0] = measure q[0];
