OPENQASM 3.0;
include "stdgates.inc";
gate rxx(_gate_p_0) _gate_q_0, _gate_q_1 {
  h _gate_q_0;
  h _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  rz(0.231) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  h _gate_q_1;
  h _gate_q_0;
}
gate rzz(_gate_p_0) _gate_q_0, _gate_q_1 {
  cx _gate_q_0, _gate_q_1;
  rz(0.231) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
}
gate unitary _gate_q_0, _gate_q_1 {
  U(pi/2, -3*pi/4, -pi) _gate_q_0;
  U(1.1848501784590992, -0.7024688891647095, -0.41842732511528613) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi, -0.46111590409721526, 1.1096804226976813) _gate_q_0;
  U(1.618059084520762, 1.8106488484429608, -1.9581592237976881) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, -pi, -pi/4) _gate_q_0;
  U(2.2125215592342102, 2.2977102204433244, -2.6523928776916197) _gate_q_1;
}
gate unitary_0 _gate_q_0, _gate_q_1 {
  U(pi/2, -pi, 0) _gate_q_0;
  U(pi/2, 0, 2.5802058126763807) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(2.9105926535897932, 0, pi/2) _gate_q_0;
  U(1.674935192019934, 1.9197876672336571, -2.1456856246166884) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, 0, -pi/2) _gate_q_0;
  U(pi/2, -1.0094094858814842, -pi) _gate_q_1;
}
gate unitary_1 _gate_q_0, _gate_q_1 {
  U(pi/2, -3*pi/4, -pi) _gate_q_0;
  U(1.1848501784590992, -0.7024688891647095, -0.41842732511528613) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi, -0.46111590409721526, 1.1096804226976813) _gate_q_0;
  U(1.674935192019934, 1.9197876672336571, -2.1456856246166884) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, -pi, -pi/4) _gate_q_0;
  U(2.2125215592342102, 2.2977102204433235, -2.6523928776916197) _gate_q_1;
}
gate unitary_2 _gate_q_0, _gate_q_1 {
  U(pi, -0.3805063771123649, -0.3805063771123649) _gate_q_0;
  U(0.5613868409134124, pi/2, -pi/2) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(2.2175926535897936, 0, pi/2) _gate_q_0;
  U(1.750842047811926, 2.017299029606418, -2.3276478046991134) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(0, 0.3420528536178251, 1.2287434731770723) _gate_q_0;
  U(1.0094094858814837, pi/2, -pi/2) _gate_q_1;
}
gate unitary_3 _gate_q_0, _gate_q_1 {
  U(pi/2, -pi, 0) _gate_q_0;
  U(pi/2, 0, 2.5802058126763807) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(2.6795926535897934, 0, pi/2) _gate_q_0;
  U(1.8427623633644894, 2.100854809510113, -2.5046858108379393) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, 0, -pi/2) _gate_q_0;
  U(pi/2, -1.0094094858814842, -pi) _gate_q_1;
}
gate unitary_4 _gate_q_0, _gate_q_1 {
  U(pi/2, -3*pi/4, -pi) _gate_q_0;
  U(1.1848501784590992, -0.7024688891647095, -0.41842732511528613) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi, -0.46111590409721526, 1.1096804226976813) _gate_q_0;
  U(1.674935192019934, 1.9197876672336571, -2.1456856246166884) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, -pi, -pi/4) _gate_q_0;
  U(2.2125215592342102, 2.2977102204433235, -2.6523928776916197) _gate_q_1;
}
gate unitary_5 _gate_q_0, _gate_q_1 {
  U(pi, 0, -pi) _gate_q_0;
  U(0.5613868409134123, pi/2, pi/2) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(2.4485926535897935, 0, pi/2) _gate_q_0;
  U(1.674935192019934, 1.9197876672336571, -2.1456856246166884) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(0, 2.187113697835974, 2.5252752825487157) _gate_q_0;
  U(1.0094094858814837, -pi/2, -pi/2) _gate_q_1;
}
bit[1] c;
qubit[4] q;
rxx(0.231) q[1], q[2];
rzz(0.231) q[1], q[2];
rzz(0.231) q[1], q[2];
rxx(0.231) q[0], q[1];
rzz(0.231) q[0], q[1];
rxx(0.231) q[2], q[3];
unitary q[1], q[2];
rzz(0.231) q[0], q[1];
rzz(0.231) q[2], q[3];
rxx(0.231) q[1], q[2];
rxx(0.231) q[2], q[3];
rzz(0.231) q[1], q[2];
unitary_0 q[0], q[1];
rzz(0.231) q[2], q[3];
rzz(0.231) q[2], q[3];
rzz(0.231) q[2], q[3];
rzz(0.231) q[2], q[3];
rzz(0.231) q[2], q[3];
rzz(0.231) q[2], q[3];
rxx(0.231) q[2], q[3];
rzz(0.231) q[1], q[2];
rxx(0.231) q[1], q[2];
rxx(0.231) q[1], q[2];
rzz(0.231) q[2], q[3];
rzz(0.231) q[1], q[2];
rzz(0.231) q[1], q[2];
rxx(0.231) q[0], q[1];
rxx(0.231) q[2], q[3];
rzz(0.231) q[1], q[2];
rxx(0.231) q[0], q[1];
rzz(0.231) q[0], q[1];
rzz(0.231) q[0], q[1];
rzz(0.231) q[2], q[3];
rzz(0.231) q[2], q[3];
rzz(0.231) q[1], q[2];
rzz(0.231) q[0], q[1];
unitary_1 q[1], q[2];
rxx(0.231) q[0], q[1];
rzz(0.231) q[1], q[2];
rzz(0.231) q[1], q[2];
unitary_2 q[0], q[1];
unitary_3 q[2], q[3];
rxx(0.231) q[1], q[2];
unitary_4 q[0], q[1];
rxx(0.231) q[1], q[2];
rzz(0.231) q[1], q[2];
rxx(0.231) q[1], q[2];
rzz(0.231) q[2], q[3];
rzz(0.231) q[1], q[2];
rzz(0.231) q[0], q[1];
rxx(0.231) q[0], q[1];
rxx(0.231) q[0], q[1];
rzz(0.231) q[0], q[1];
rxx(0.231) q[2], q[3];
rzz(0.231) q[2], q[3];
rzz(0.231) q[1], q[2];
rxx(0.231) q[0], q[1];
unitary q[2], q[3];
rzz(0.231) q[1], q[2];
rxx(0.231) q[0], q[1];
unitary_5 q[2], q[3];
rxx(0.231) q[1], q[2];
rxx(0.231) q[0], q[1];
rxx(0.231) q[1], q[2];
rxx(0.231) q[0], q[1];
rxx(0.231) q[1], q[2];
rzz(0.231) q[1], q[2];
rzz(0.231) q[1], q[2];
rzz(0.231) q[0], q[1];
rzz(0.231) q[0], q[1];
rzz(0.231) q[2], q[3];
rxx(0.231) q[2], q[3];
rzz(0.231) q[2], q[3];
c[0] = measure q[0];
