OPENQASM 3.0;
include "stdgates.inc";
gate rxx(_gate_p_0) _gate_q_0, _gate_q_1 {
  h _gate_q_0;
  h _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  rz(0.297) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  h _gate_q_1;
  h _gate_q_0;
}
gate rzz(_gate_p_0) _gate_q_0, _gate_q_1 {
  cx _gate_q_0, _gate_q_1;
  rz(0.297) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
}
gate unitary _gate_q_0, _gate_q_1 {
  U(pi/2, -3*pi/4, -pi) _gate_q_0;
  U(1.1848501784590992, -0.7024688891647095, -0.41842732511528613) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi, -0.46111590409721526, 1.1096804226976813) _gate_q_0;
  U(1.6480661321272245, 1.8742768745898992, -2.0659844284769786) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, -pi, -pi/4) _gate_q_0;
  U(2.2125215592342102, 2.2977102204433235, -2.6523928776916197) _gate_q_1;
}
gate unitary_0 _gate_q_0, _gate_q_1 {
  U(pi/2, -pi, 0) _gate_q_0;
  U(pi/2, 0, 2.5802058126763807) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(2.8445926535897934, 0, pi/2) _gate_q_0;
  U(1.738944376797236, 2.0041755239816466, -2.301985313644523) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, 0, -pi/2) _gate_q_0;
  U(pi/2, -1.0094094858814837, -pi) _gate_q_1;
}
gate unitary_1 _gate_q_0, _gate_q_1 {
  U(pi/2, -3*pi/4, -pi) _gate_q_0;
  U(1.1848501784590992, -0.7024688891647095, -0.41842732511528613) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi, -0.46111590409721526, 1.1096804226976813) _gate_q_0;
  U(1.738944376797236, 2.0041755239816466, -2.301985313644523) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, -pi, -pi/4) _gate_q_0;
  U(2.2125215592342102, 2.2977102204433235, -2.6523928776916197) _gate_q_1;
}
gate unitary_2 _gate_q_0, _gate_q_1 {
  U(pi, 0, -pi) _gate_q_0;
  U(0.5613868409134123, pi/2, pi/2) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(1.9535926535897934, 0, pi/2) _gate_q_0;
  U(1.857015018361137, 2.111538313755408, -2.5296783839143435) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(0, 1.7302303646815265, 2.982158615703163) _gate_q_0;
  U(1.0094094858814837, -pi/2, -pi/2) _gate_q_1;
}
gate unitary_3 _gate_q_0, _gate_q_1 {
  U(pi/2, -pi, 0) _gate_q_0;
  U(pi/2, 0, 2.5802058126763807) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(2.5475926535897937, 0, pi/2) _gate_q_0;
  U(1.9954835188150246, 2.192121274523659, -2.75349273346305) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, 0, -pi/2) _gate_q_0;
  U(pi/2, -1.0094094858814837, -pi) _gate_q_1;
}
gate unitary_4 _gate_q_0, _gate_q_1 {
  U(pi/2, -3*pi/4, -pi) _gate_q_0;
  U(1.1848501784590992, -0.7024688891647095, -0.41842732511528613) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi, -0.46111590409721526, 1.1096804226976813) _gate_q_0;
  U(1.738944376797236, 2.0041755239816466, -2.301985313644523) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, -pi, -pi/4) _gate_q_0;
  U(2.2125215592342102, 2.2977102204433235, -2.6523928776916197) _gate_q_1;
}
gate unitary_5 _gate_q_0, _gate_q_1 {
  U(pi, 0, -pi) _gate_q_0;
  U(0.5613868409134123, pi/2, pi/2) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(2.2505926535897935, 0, pi/2) _gate_q_0;
  U(1.7389443767972361, 2.0041755239816466, -2.301985313644523) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(0, 2.1112158270654824, 2.6011731533192073) _gate_q_0;
  U(1.0094094858814837, -pi/2, -pi/2) _gate_q_1;
}
bit[1] c;
qubit[4] q;
rxx(0.297) q[1], q[2];
rzz(0.297) q[1], q[2];
rzz(0.297) q[1], q[2];
rxx(0.297) q[0], q[1];
rzz(0.297) q[0], q[1];
rxx(0.297) q[2], q[3];
unitary q[1], q[2];
rzz(0.297) q[0], q[1];
rzz(0.297) q[2], q[3];
rxx(0.297) q[1], q[2];
rxx(0.297) q[2], q[3];
rzz(0.297) q[1], q[2];
unitary_0 q[0], q[1];
rzz(0.297) q[2], q[3];
rzz(0.297) q[2], q[3];
rzz(0.297) q[2], q[3];
rzz(0.297) q[2], q[3];
rzz(0.297) q[2], q[3];
rzz(0.297) q[2], q[3];
rxx(0.297) q[2], q[3];
rzz(0.297) q[1], q[2];
rxx(0.297) q[1], q[2];
rxx(0.297) q[1], q[2];
rzz(0.297) q[2], q[3];
rzz(0.297) q[1], q[2];
rzz(0.297) q[1], q[2];
rxx(0.297) q[0], q[1];
rxx(0.297) q[2], q[3];
rzz(0.297) q[1], q[2];
rxx(0.297) q[0], q[1];
rzz(0.297) q[0], q[1];
rzz(0.297) q[0], q[1];
rzz(0.297) q[2], q[3];
rzz(0.297) q[2], q[3];
rzz(0.297) q[1], q[2];
rzz(0.297) q[0], q[1];
unitary_1 q[1], q[2];
rxx(0.297) q[0], q[1];
rzz(0.297) q[1], q[2];
rzz(0.297) q[1], q[2];
unitary_2 q[0], q[1];
unitary_3 q[2], q[3];
rxx(0.297) q[1], q[2];
unitary_4 q[0], q[1];
rxx(0.297) q[1], q[2];
rzz(0.297) q[1], q[2];
rxx(0.297) q[1], q[2];
rzz(0.297) q[2], q[3];
rzz(0.297) q[1], q[2];
rzz(0.297) q[0], q[1];
rxx(0.297) q[0], q[1];
rxx(0.297) q[0], q[1];
rzz(0.297) q[0], q[1];
rxx(0.297) q[2], q[3];
rzz(0.297) q[2], q[3];
rzz(0.297) q[1], q[2];
rxx(0.297) q[0], q[1];
unitary q[2], q[3];
rzz(0.297) q[1], q[2];
rxx(0.297) q[0], q[1];
unitary_5 q[2], q[3];
rxx(0.297) q[1], q[2];
rxx(0.297) q[0], q[1];
rxx(0.297) q[1], q[2];
rxx(0.297) q[0], q[1];
rxx(0.297) q[1], q[2];
rzz(0.297) q[1], q[2];
rzz(0.297) q[1], q[2];
rzz(0.297) q[0], q[1];
rzz(0.297) q[0], q[1];
rzz(0.297) q[2], q[3];
rxx(0.297) q[2], q[3];
rzz(0.297) q[2], q[3];
c[0] = measure q[0];
