OPENQASM 3.0;
include "stdgates.inc";
gate rxx(_gate_p_0) _gate_q_0, _gate_q_1 {
  h _gate_q_0;
  h _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  rz(0.111) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  h _gate_q_1;
  h _gate_q_0;
}
gate rzz(_gate_p_0) _gate_q_0, _gate_q_1 {
  cx _gate_q_0, _gate_q_1;
  rz(0.111) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
}
gate unitary _gate_q_0, _gate_q_1 {
  U(pi/2, -3*pi/4, -pi) _gate_q_0;
  U(1.1848501784590992, -0.7024688891647095, -0.41842732511528613) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi, -0.46111590409721526, 1.1096804226976813) _gate_q_0;
  U(1.5818560030598667, 1.6882893330647688, -1.758291424399684) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, -pi, -pi/4) _gate_q_0;
  U(2.2125215592342102, 2.2977102204433244, -2.6523928776916197) _gate_q_1;
}
gate unitary_0 _gate_q_0, _gate_q_1 {
  U(pi/2, -pi, 0) _gate_q_0;
  U(pi/2, 0, 2.5802058126763807) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(3.0305926535897934, 0, pi/2) _gate_q_0;
  U(1.595555030856442, 1.7457588671862743, -1.851252735455315) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, 0, -pi/2) _gate_q_0;
  U(pi/2, -1.0094094858814842, -pi) _gate_q_1;
}
gate unitary_1 _gate_q_0, _gate_q_1 {
  U(pi/2, -3*pi/4, -pi) _gate_q_0;
  U(1.1848501784590992, -0.7024688891647095, -0.41842732511528613) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi, -0.46111590409721526, 1.1096804226976813) _gate_q_0;
  U(1.595555030856442, 1.7457588671862743, -1.851252735455315) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, -pi, -pi/4) _gate_q_0;
  U(2.2125215592342102, 2.2977102204433244, -2.6523928776916197) _gate_q_1;
}
gate unitary_2 _gate_q_0, _gate_q_1 {
  U(pi, 0, -pi) _gate_q_0;
  U(0.5613868409134123, pi/2, pi/2) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(2.6975926535897927, 0, pi/2) _gate_q_0;
  U(1.614505226975356, 1.801740614045908, -1.9433268519686875) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(0, 2.085924760805103, 2.626464219579587) _gate_q_0;
  U(1.0094094858814837, -pi/2, -pi/2) _gate_q_1;
}
gate unitary_3 _gate_q_0, _gate_q_1 {
  U(pi/2, -pi, 0) _gate_q_0;
  U(pi/2, 0, 2.5802058126763807) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(2.9195926535897936, 0, pi/2) _gate_q_0;
  U(1.6384907906203434, 1.855807201199137, -2.034306657770683) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, 0, -pi/2) _gate_q_0;
  U(pi/2, -1.0094094858814837, -pi) _gate_q_1;
}
gate unitary_4 _gate_q_0, _gate_q_1 {
  U(pi/2, -3*pi/4, -pi) _gate_q_0;
  U(1.1848501784590992, -0.7024688891647095, -0.41842732511528613) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi, -0.46111590409721526, 1.1096804226976813) _gate_q_0;
  U(1.595555030856442, 1.7457588671862743, -1.851252735455315) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(pi/2, -pi, -pi/4) _gate_q_0;
  U(2.2125215592342102, 2.2977102204433244, -2.6523928776916197) _gate_q_1;
}
gate unitary_5 _gate_q_0, _gate_q_1 {
  U(pi, 0, -pi) _gate_q_0;
  U(0.5613868409134123, pi/2, pi/2) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(2.8085926535897934, 0, pi/2) _gate_q_0;
  U(1.595555030856442, 1.7457588671862743, -1.851252735455315) _gate_q_1;
  cx _gate_q_0, _gate_q_1;
  U(0, 1.8027524598072873, 2.9096365205774024) _gate_q_0;
  U(1.0094094858814837, -pi/2, -pi/2) _gate_q_1;
}
bit[1] c;
qubit[4] q;
rxx(0.111) q[1], q[2];
rzz(0.111) q[1], q[2];
rzz(0.111) q[1], q[2];
rxx(0.111) q[0], q[1];
rzz(0.111) q[0], q[1];
rxx(0.111) q[2], q[3];
unitary q[1], q[2];
rzz(0.111) q[0], q[1];
rzz(0.111) q[2], q[3];
rxx(0.111) q[1], q[2];
rxx(0.111) q[2], q[3];
rzz(0.111) q[1], q[2];
unitary_0 q[0], q[1];
rzz(0.111) q[2], q[3];
rzz(0.111) q[2], q[3];
rzz(0.111) q[2], q[3];
rzz(0.111) q[2], q[3];
rzz(0.111) q[2], q[3];
rzz(0.111) q[2], q[3];
rxx(0.111) q[2], q[3];
rzz(0.111) q[1], q[2];
rxx(0.111) q[1], q[2];
rxx(0.111) q[1], q[2];
rzz(0.111) q[2], q[3];
rzz(0.111) q[1], q[2];
rzz(0.111) q[1], q[2];
rxx(0.111) q[0], q[1];
rxx(0.111) q[2], q[3];
rzz(0.111) q[1], q[2];
rxx(0.111) q[0], q[1];
rzz(0.111) q[0], q[1];
rzz(0.111) q[0], q[1];
rzz(0.111) q[2], q[3];
rzz(0.111) q[2], q[3];
rzz(0.111) q[1], q[2];
rzz(0.111) q[0], q[1];
unitary_1 q[1], q[2];
rxx(0.111) q[0], q[1];
rzz(0.111) q[1], q[2];
rzz(0.111) q[1], q[2];
unitary_2 q[0], q[1];
unitary_3 q[2], q[3];
rxx(0.111) q[1], q[2];
unitary_4 q[0], q[1];
rxx(0.111) q[1], q[2];
rzz(0.111) q[1], q[2];
rxx(0.111) q[1], q[2];
rzz(0.111) q[2], q[3];
rzz(0.111) q[1], q[2];
rzz(0.111) q[0], q[1];
rxx(0.111) q[0], q[1];
rxx(0.111) q[0], q[1];
rzz(0.111) q[0], q[1];
rxx(0.111) q[2], q[3];
rzz(0.111) q[2], q[3];
rzz(0.111) q[1], q[2];
rxx(0.111) q[0], q[1];
unitary q[2], q[3];
rzz(0.111) q[1], q[2];
rxx(0.111) q[0], q[1];
unitary_5 q[2], q[3];
rxx(0.111) q[1], q[2];
rxx(0.111) q[0], q[1];
rxx(0.111) q[1], q[2];
rxx(0.111) q[0], q[1];
rxx(0.111) q[1], q[2];
rzz(0.111) q[1], q[2];
rzz(0.111) q[1], q[2];
rzz(0.111) q[0], q[1];
rzz(0.111) q[0], q[1];
rzz(0.111) q[2], q[3];
rxx(0.111) q[2], q[3];
rzz(0.111) q[2], q[3];
c[0] = measure q[0];
